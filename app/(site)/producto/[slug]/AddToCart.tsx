  ...
  return (
    <>
      ...
      {product.stock === "a_pedido" && (
        <p style={{ fontSize: 13, color: "var(--wood)", fontWeight: 600, margin: "-8px 0 18px" }}>Se fabrica a pedido{product.tiempoFabricacion ? ` · ${product.tiempoFabricacion}` : ""}.</p>
      )}
      {product.stock === "sin_stock" && <p style={{ fontSize: 13, color: "var(--accent)", fontWeight: 600, margin: "-8px 0 18px" }}>Sin stock por el momento. Consultanos cuándo vuelve a estar disponible.</p>}

      <div className="row" style={{ maxWidth: 340, justifyContent: "flex-start" }}>
        <Button
          full
          disabled={product.stock === "sin_stock"}
          onClick={() => {
            add(product, hasOptions ? { price, label: selectionLabel(product.options, selection), values: selection } : undefined);
            setAdded(true);
            setTimeout(() => setAdded(false), 1600);
          }}
          icon={<Icon name={added ? "check" : "shopping-cart"} size={16} color="var(--white)" />}
        >
          {product.stock === "sin_stock" ? "Sin stock" : added ? "¡Agregado al carrito!" : "Agregar al carrito"}
        </Button>
