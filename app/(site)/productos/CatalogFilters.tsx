...
export function CatalogFilters({ products, categories }: { products: Product[]; categories: Category[] }) {
  ...
  // La categoría viaja como slug (?categoria=paneles-ranurados, ?categoria=todos); sin parámetro o con un slug desconocido se muestra "Todos".
  const catSlug = searchParams.get("categoria") || ALL;
  const cat = categories.find((c) => slugify(c.name) === catSlug)?.name ?? "Todos";
  ...
  // Entrar a /productos a secas equivale a "Todos": la URL lo refleja igual que en las demás categorías.
  const hasCategoryParam = searchParams.has("categoria");
  useEffect(() => {
    if (!hasCategoryParam) setParams({ categoria: ALL }, false);
  }, [hasCategoryParam, setParams]);

  const setCat = (v: string) => setParams({ categoria: v === "Todos" ? ALL : slugify(v) });
    ...
    <div data-screen-label="Catálogo">
      <div className="wrap" style={{ paddingBottom: 30 }}>
        <div className="center" style={{ marginBottom: 30 }}>
          <SectionTitle eyebrow={cat === "Todos" ? "Catálogo completo" : "Catálogo"} title={cat === "Todos" ? "Todos los productos" : cat} size="lg" style={{ textAlign: "center", marginBottom: 18 }} />
          <div style={{ width: "100%", maxWidth: 440 }}>
            <Input placeholder={cat === "Todos" ? "Buscar en todos los productos" : `Buscar en ${cat}`} value={q} onChange={(e) => setQ(e.target.value)} mono={false} />
          </div>
        </div>
