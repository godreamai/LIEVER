import Link from "next/link";
import { AppHeader } from "@/components/Header";
import { Analytics } from "@/components/Analytics";
import { BrandLogo } from "@/components/ui";
import { WhatsAppFab } from "@/components/ds/WhatsAppFab";
import { Icon } from "@/components/ds/Icon";
import { WA, PHONE, PHONE_RAW, INSTAGRAM, ADDRESS, MAPS_URL, HOURS_WEEKDAY, HOURS_SAT } from "@/lib/data";
import { NAV_LINKS } from "@/lib/nav";

const FOOTER_COL_LABEL: React.CSSProperties = {
  fontFamily: "var(--font-body)",
  fontSize: 11,
  fontWeight: 600,
  letterSpacing: ".08em",
  textTransform: "uppercase",
  color: "rgba(255,252,246,.5)",
  marginBottom: 4,
};

export default function SiteLayout({ children }: { children: React.ReactNode }) {
  const explora = NAV_LINKS.filter((l) => l.group === "explora");
  const marca = NAV_LINKS.filter((l) => l.group === "marca");

  return (
    <div className="site-shell">
      <Analytics />
      <AppHeader />
      <div className="site-main">{children}</div>

      <footer style={{
        background: "var(--ink)",
        color: "var(--white)",
      }}>
        {/* main footer body */}
        <div className="site-footer-container">
          {/* col 1 — brand */}
          <div className="site-footer-col">
            <div>
              <BrandLogo height={34} tone="inverse" />
            </div>
            <p style={{
              fontSize: 13,
              color: "rgba(255,252,246,.6)",
              lineHeight: 1.65,
              margin: 0,
              maxWidth: 260,
            }}>
              Diseño y fabricación de mobiliario y soluciones pensadas para tu espacio.
            </p>
            <a
              href={INSTAGRAM}
              target="_blank"
              rel="noreferrer"
              style={{
                display: "inline-flex",
                alignItems: "center",
                gap: 8,
                fontSize: 13,
                color: "rgba(255,252,246,.75)",
                fontFamily: "var(--font-body)",
                textDecoration: "none",
              }}
            >
              <Icon name="instagram" size={14} color="var(--accent)" />
              @liever.sannicolas
            </a>
          </div>

          {/* col 2 — explorá */}
          <div className="site-footer-col">
            <span style={FOOTER_COL_LABEL}>Explorá</span>
            {explora.map((l) => (
              <Link key={l.id} href={l.href} className="footer-nav-link">
                {l.label}
              </Link>
            ))}
          </div>

          {/* col 3 — marca */}
          <div className="site-footer-col">
            <span style={FOOTER_COL_LABEL}>Marca</span>
            {marca.map((l) => (
              <Link key={l.id} href={l.href} className="footer-nav-link">
                {l.label}
              </Link>
            ))}
          </div>

          {/* col 4 — contacto & taller */}
          <div className="site-footer-col">
            <span style={FOOTER_COL_LABEL}>Taller</span>
            <div className="site-footer-contacts">
              <a
                href={MAPS_URL}
                target="_blank"
                rel="noreferrer"
                style={{
                  display: "inline-flex",
                  alignItems: "flex-start",
                  gap: 8,
                  fontSize: 13,
                  color: "rgba(255,252,246,.75)",
                  fontFamily: "var(--font-body)",
                  textDecoration: "none",
                  transition: "color .15s ease",
                }}
              >
                <Icon name="map-pin" size={14} color="var(--accent)" style={{ marginTop: 2 }} />
                {ADDRESS}
              </a>

              <span
                style={{
                  display: "inline-flex",
                  alignItems: "center",
                  gap: 8,
                  fontSize: 12,
                  color: "rgba(255,252,246,.6)",
                  fontFamily: "var(--font-body)",
                }}
              >
                <Icon name="clock" size={14} color="var(--accent)" />
                {HOURS_WEEKDAY} · {HOURS_SAT}
              </span>

              <a
                href={`tel:${PHONE_RAW}`}
                style={{
                  display: "inline-flex",
                  alignItems: "center",
                  gap: 8,
                  fontSize: 13,
                  color: "rgba(255,252,246,.75)",
                  fontFamily: "var(--font-body)",
                  textDecoration: "none",
                }}
              >
                <Icon name="phone" size={14} color="var(--accent)" />
                {PHONE}
              </a>
            </div>
          </div>
        </div>

        {/* divider */}
        <div style={{
          borderTop: "1px solid rgba(255,252,246,.1)",
          maxWidth: 1200,
          margin: "0 auto",
        }} />

        {/* bottom bar */}
        <div className="site-footer-bottom">
          <span style={{
            fontSize: 12,
            color: "rgba(255,252,246,.4)",
            fontFamily: "var(--font-body)",
          }}>
            © {new Date().getFullYear()} Liever — San Nicolás de los Arroyos
          </span>
          <Link
            href="/admin"
            className="footer-admin-link"
          >
            <Icon name="settings" size={12} color="currentColor" />
            Panel de administración
          </Link>
        </div>
      </footer>

      <WhatsAppFab href={WA + encodeURIComponent("Hola Liever! Me contacto desde la web para hacer una consulta.")} />
    </div>
  );
}
